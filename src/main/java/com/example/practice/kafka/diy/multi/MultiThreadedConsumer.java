 ...
 * 1、创建Consumer；
 * 2、读取订阅分区的消息；
 * 3、将消息按照不同分区进行归组分发给不同的线程；
 * 4、暂停这些分区的后续消费，同时等待Worker线程完成消息处理；
 * 5、提交这些分区的位移；
 * 6、恢复这些分区的消费。
 */
public class MultiThreadedConsumer {
  ...
  private final static Executor executor = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors() * 10, r -> {
